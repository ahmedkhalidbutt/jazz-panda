...
(function(g, k) {
	'function' === typeof define && define.amd
		? define([], k)
		: 'object' === typeof exports ? (module.exports = k('Selectr')) : (g.Selectr = k('Selectr'));
})(this, function(g) {
	function k(a, c) {
	...
	l.prototype = {
		...
		emit: function(a) {
			...
			if (!1 !== a in this._events)
				for (var c = 0; c < this._events[a].length; c++)
	...
	var b = {
			extend: function(a, c) {
				for (var e in c)
					if (c.hasOwnProperty(e)) {
						var d = c[e];
						d && '[object Object]' === Object.prototype.toString.call(d)
							? ((a[e] = a[e] || {}), b.extend(a[e], d))
							: (a[e] = d);
					}
				return a;
			},
			each: function(a, c, b) {
				if ('[object Object]' === Object.prototype.toString.call(a))
					for (var d in a) Object.prototype.hasOwnProperty.call(a, d) && c.call(b, d, a[d], a);
				else {
					d = 0;
			...
			createElement: function(a, c) {
				var b = document,
					d = b.createElement(a);
				if (c && '[object Object]' === Object.prototype.toString.call(c))
					for (var f in c)
						if (f in d) d[f] = c[f];
		...
		p = function() {
			if (this.items.length) {
				...
				a.childElementCount &&
					(b.removeClass(this.items[this.navIndex], 'active'),
					(this.navIndex = (a.querySelector('.selectr-option.selected') ||
						a.querySelector('.selectr-option')).idx),
		...
		t = function(a) {
			this.container.contains(a.target) ||
				(!this.opened && !b.hasClass(this.container, 'notice')) ||
				this.close();
		},
		m = function(a, c) {
		...
		u = function() {
			...
			k(this.config, 'width') &&
				(b.isInt(this.config.width)
					? (this.width = this.config.width + 'px')
					: 'auto' === this.config.width
						? (this.width = '100%')
						: b.includes(this.config.width, '%') && (this.width = this.config.width));
			this.container = b.createElement('div', { class: 'selectr-container' });
			this.config.customClass && b.addClass(this.container, this.config.customClass);
			this.mobileDevice
				? b.addClass(this.container, 'selectr-mobile')
				: b.addClass(this.container, 'selectr-desktop');
			this.el.tabIndex = -1;
			this.config.nativeDropdown || this.mobileDevice
				? b.addClass(this.el, 'selectr-visible')
				: b.addClass(this.el, 'selectr-hidden');
			...
			this.label = b.createElement(this.el.multiple ? 'ul' : 'span', { class: 'selectr-label' });
			...
			this.el.multiple &&
				(b.addClass(this.label, 'selectr-tags'),
				b.addClass(this.container, 'multiple'),
				(this.tags = []),
				(this.selectedValues = this.getSelectedProperties('value')),
				(this.selectedIndexes = this.getSelectedProperties('idx')));
			this.selected.appendChild(this.label);
			this.config.clearable &&
				((this.selectClear = b.createElement('button', { class: 'selectr-clear', type: 'button' })),
				this.container.appendChild(this.selectClear),
				b.addClass(this.container, 'clearable'));
			...
			this.config.searchable &&
				...
				(this.inputClear = b.createElement('button', { class: 'selectr-input-clear', type: 'button' })),
				(this.inputContainer = b.createElement('div', { class: 'selectr-input-container' })),
				this.inputContainer.appendChild(this.input),
				this.inputContainer.appendChild(this.inputClear),
				a.appendChild(this.inputContainer));
			...
			if (this.config.data && Array.isArray(this.config.data)) {
				...
				b.each(
					...
					function(a, c) {
						k(c, 'children')
							? ((f = b.createElement('optgroup', { label: c.text })),
								(e = b.createElement('ul', {
									class: 'selectr-optgroup',
									role: 'group',
								...
								b.each(
									...
									function(a, b) {
										h = new Option(
											...
											b.value,
											!1,
											b.hasOwnProperty('selected') && !0 === b.selected
										);
							...
							: ((h = new Option(c.text, c.value, !1, c.hasOwnProperty('selected') && !0 === c.selected)),
								(h.disabled = k(c, 'disabled')),
								this.options.push(h),
								(h.idx = d),
								m.call(this, h, c),
								(this.data[d] = c),
		...
		v = function(a) {
			...
			if (this.items.length && this.opened && b.includes([ 13, 38, 40 ], a.which)) {
				a.preventDefault();
				if (13 === a.which)
					return this.noResults || (this.config.taggable && 0 < this.input.value.length)
						? !1
				...
				for (
					this.navigating = !0;
					b.hasClass(this.items[this.navIndex], 'disabled') ||
					b.hasClass(this.items[this.navIndex], 'excluded');

				) {
					if (0 < this.navIndex && this.navIndex < this.items.length - 1)
						d ? this.navIndex++ : this.navIndex--;
					else {
						this.navIndex = e;
						break;
					}
				...
				d
					? (0 === this.navIndex
							...
							: a.top + a.height > this.optsRect.top + this.optsRect.height &&
								(this.tree.scrollTop += a.top + a.height - (this.optsRect.top + this.optsRect.height)),
						this.navIndex === this.tree.childElementCount - 1 && this.requiresPagination && r.call(this))
					: 0 === this.navIndex
						? (this.tree.scrollTop = 0)
						: 0 > a.top - this.optsRect.top && (this.tree.scrollTop += a.top - this.optsRect.top);
		...
		q = function() {
			if (this.config.searchable || this.config.taggable)
				(this.input.value = null),
					(this.searching = !1),
					this.config.searchable && b.removeClass(this.inputContainer, 'active'),
					b.hasClass(this.container, 'notice') &&
						(b.removeClass(this.container, 'notice'),
						b.addClass(this.container, 'open'),
						this.input.focus()),
					b.each(
						this.items,
						function(a, c) {
	...
	g = function(a, b) {
		...
		if ('select' !== this.el.nodeName.toLowerCase())
			throw Error('The element you passed to Selectr is not a HTMLSelectElement.');
		this.render(b);
	};
	g.prototype.render = function(a) {
		if (!this.rendered) {
			...
			this.customOption =
				this.config.hasOwnProperty('renderOption') && 'function' === typeof this.config.renderOption;
			this.customSelected =
				this.config.hasOwnProperty('renderSelection') && 'function' === typeof this.config.renderSelection;
	...
	g.prototype.bindEvents = function() {
		...
		if (this.config.nativeDropdown || this.mobileDevice) {
			...
			this.el.addEventListener('change', function(d) {
				a.el.multiple
					? ((d = a.getSelectedProperties('idx')),
						(d = c(a.selectedIndexes, d)),
						b.each(
							d[0],
							function(b, d) {
								a.select(d);
					...
					: -1 < a.el.selectedIndex && a.select(a.el.selectedIndex);
			});
		}
		this.container.addEventListener('keydown', function(b) {
			'Escape' === b.key && a.close();
			'Enter' === b.key &&
				a.selected === document.activeElement &&
				'undefined' !== typeof a.el.form.submit &&
				a.el.form.submit();
			(' ' !== b.key && 'ArrowUp' !== b.key && 'ArrowDown' !== b.key) ||
				a.selected !== document.activeElement ||
				(setTimeout(function() {
					a.toggle();
				}, 200),
				a.config.nativeDropdown &&
					setTimeout(function() {
						a.el.focus();
					}, 200));
		});
		...
		if (this.config.nativeKeyboard) {
			...
			this.selected.addEventListener('keydown', function(b) {
				if (!(a.disabled || a.selected !== document.activeElement || b.altKey || b.ctrlKey || b.metaKey))
					if (' ' === b.key || (!a.opened && -1 < [ 'Enter', 'ArrowUp', 'ArrowDown' ].indexOf(b.key)))
						a.toggle(), b.preventDefault(), b.stopPropagation();
					else if (
						2 >= b.key.length &&
						String[String.fromCodePoint ? 'fromCodePoint' : 'fromCharCode'](
							b.key[String.codePointAt ? 'codePointAt' : 'charCodeAt'](0)
						) === b.key
					) {
						if (a.config.multiple)
							a.open(),
								a.config.searchable && ((a.input.value = b.key), a.input.focus(), a.search(null, !0));
						else {
							e += b.key;
							var c = a.search(e, !0);
		...
		this.tree.addEventListener('click', function(c) {
			...
			d &&
				!b.hasClass(d, 'disabled') &&
				(b.hasClass(d, 'selected')
					? (a.el.multiple || (!a.el.multiple && a.config.allowDeselect)) && a.deselect(d.idx)
					: a.select(d.idx),
				a.opened && !a.el.multiple && a.close());
			c.preventDefault();
			c.stopPropagation();
		});
		this.tree.addEventListener('mouseover', function(c) {
			b.hasClass(c.target, 'selectr-option') &&
				!b.hasClass(c.target, 'disabled') &&
				(b.removeClass(a.items[a.navIndex], 'active'),
				b.addClass(c.target, 'active'),
				(a.navIndex = [].slice.call(a.items).indexOf(c.target)));
		});
		this.config.searchable &&
			(this.input.addEventListener('focus', function(b) {
				a.searching = !0;
			}),
		...
		this.config.taggable &&
			this.input.addEventListener('keyup', function(c) {
				...
				if (a.config.taggable && this.value.length) {
					...
					if (13 === c.which || b.includes(a.tagSeperators, c.key))
						b.each(a.tagSeperators, function(a, b) {
							d = d.replace(b, '');
						}),
							a.add({ value: d, text: d, selected: !0 }, !0)
								? (a.close(), q.call(a))
								: ((this.value = ''), a.setMessage(a.config.messages.tagDuplicate));
				}
			});
		...
		this.requiresPagination &&
			((this.paginateItems = b.debounce(function() {
				r.call(this);
			}, 50)),
	...
	g.prototype.setSelected = function(a) {
		this.config.data ||
			...
			-1 < this.selectedIndex && this.select(this.selectedIndex));
			...
			b.each(
				...
				function(a, d) {
					k(d, 'children')
						...
						: (d.hasOwnProperty('selected') && !0 === d.selected && this.select(c), c++);
				},
				this
			);
		}
	...
	g.prototype.destroy = function() {
		this.rendered &&
			...
			b.removeClass(this.el, 'selectr-hidden'),
	...
	g.prototype.change = function(a) {
		var c = this.items[a],
			e = this.options[a];
		e.disabled ||
			(e.selected && b.hasClass(c, 'selected') ? this.deselect(a) : this.select(a),
	...
	g.prototype.select = function(a) {
		...
		if (this.el.multiple) {
			...
			if (this.config.maxSelections && this.tags.length === this.config.maxSelections)
				return (
					this.setMessage(this.config.messages.maxSelections.replace('{max}', this.config.maxSelections), !0),
					!1
				);
		...
		'createEvent' in document
			? ((c = document.createEvent('HTMLEvents')), c.initEvent('change', !0, !0), this.el.dispatchEvent(c))
	...
	g.prototype.deselect = function(a, c) {
		...
		'createEvent' in document
			? ((e = document.createEvent('HTMLEvents')), e.initEvent('change', !0, !0), this.el.dispatchEvent(e))
			: this.el.fireEvent('onchange');
	};
	...
	g.prototype.remove = function(a) {
		...
		Array.isArray(a)
			? b.each(
					...
					function(a, e) {
						b.isInt(e)
							? c.push(this.getOptionByIndex(e))
							: 'string' === typeof e && c.push(this.getOptionByValue(e));
					},
					this
				)
			: b.isInt(a) ? c.push(this.getOptionByIndex(a)) : 'string' === typeof a && c.push(this.getOptionByValue(a));
		if (c.length) {
			var e;
			b.each(
				c,
			...
			if (0 < a.length) {
				...
				b.each(
					...
					function(c, h) {
						...
						if (g(h.textContent.trim().toLowerCase(), a) && !h.disabled) {
							if (
								(d.push({ text: h.textContent, value: h.value }),
								e && (n(k, f, this.customOption), b.removeClass(k, 'excluded'), !this.customOption))
							) {
								var l = (l = new RegExp(a, 'i').exec(h.textContent))
									? h.textContent.replace(l[0], "<span class='selectr-match'>" + l[0] + '</span>')
									: !1;
								k.innerHTML = l;
							}
				...
				if (e) {
					if (f.childElementCount) {
						...
						this.config.taggable ||
							((this.noResults = !0), this.setMessage(this.config.messages.noResults));
	...
	g.prototype.open = function() {
		...
		this.mobileDevice || this.config.nativeDropdown
			? (b.addClass(this.container, 'native-open'),
				this.config.data &&
					b.each(
						this.options,
						function(a, b) {
	...
	g.prototype.clear = function(a) {
		this.el.multiple
			...
			: -1 < this.selectedIndex && this.deselect(this.selectedIndex, a);
	...
	g.prototype.invert = function() {
		...
		a.top + a.height + this.tree.parentNode.offsetHeight > window.innerHeight
			? (b.addClass(this.container, 'inverted'), (this.isInverted = !0))
